...
def load_seattle_dataset():
    """Load Seattle dataset only (no synthetic data)."""
    ...
    print(f"Dataset: {len(seattle_states)} days")
    print(f"State distribution: {np.unique(seattle_states, return_counts=True)}")
    return seattle_obs_scaled, seattle_states, seattle_df, scaler

def train_and_evaluate_model():
    """Train and evaluate the best HMM model."""
    print("\nTraining Best HMM Model...")
    
    # Load Seattle-only dataset
    observations, states, seattle_df, scaler = load_seattle_dataset()
...
if __name__ == "__main__":
    ...
    print("Using Seattle Weather Dataset Only")
    print("="*60)
    ...
    print(f"Using Seattle dataset with {len(results['test_states'])} test samples")
